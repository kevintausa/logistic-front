...
export default function LCLOperationPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <LCLOperationsSection />
    </div>
  );
}

function LCLOperationsSection() {
  ...
  const handleSaveItem = useCallback(async (payload) => {
    try {
      ...
      if (resp?.code === 201) {
        toast({ title: 'Solicitud creada', description: 'Se creó la solicitud LCL.' });
      } else {
  ...
  // Prefill tipoOperacion for legacy LCL
  const initialItem = useMemo(() => ({ tipoOperacion: { id: 'importacion_lcl', nombre: 'Importación LCL' } }), []);

  return (
    <>
      <OperationModuleBase
        title="LCL"
        description="Listado y gestión de operaciones LCL."
        columns={operationsColumns}
        filterFields={FILTER_FIELDS}
        fetchService={fetchOperations}
        exportService={exportOperations}
        columnsExcel={columnsExcel}
        fileName="operaciones_lcl"
        defaultFilters={{ 'tipoOperacion.id': { $in: ['importacion_lcl', 'exportacion_lcl'] } }}
