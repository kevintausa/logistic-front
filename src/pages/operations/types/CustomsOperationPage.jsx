...
export default function CustomsOperationPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <CustomsOperationsSection FILTER_FIELDS={FILTER_FIELDS} />
    </div>
  );
}

function CustomsOperationsSection({ FILTER_FIELDS }) {
  ...
  const handleSaveItem = useCallback(async (payload) => {
    try {
      ...
      if (resp?.code === 201) {
        toast({ title: 'Solicitud creada', description: 'Se creó la solicitud de Aduana.' });
      } else {
  ...
  const initialItem = useMemo(() => ({ tipoOperacion: { id: 'agenciamiento_aduanero', nombre: 'Agenciamiento aduanero' } }), []);

  return (
    <>
      <OperationModuleBase
        title="Aduana"
        description="Listado y gestión de operaciones de aduana."
        columns={operationsColumns}
        filterFields={FILTER_FIELDS}
        fetchService={fetchOperations}
        exportService={exportOperations}
        columnsExcel={columnsExcel}
        fileName="operaciones_aduana"
        defaultFilters={{ 'tipoOperacion.id': 'agenciamiento_aduanero' }}
