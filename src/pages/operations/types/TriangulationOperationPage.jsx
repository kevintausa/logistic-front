...
export default function TriangulationOperationPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <TriangulationOperationsSection FILTER_FIELDS={FILTER_FIELDS} />
    </div>
  );
}

function TriangulationOperationsSection({ FILTER_FIELDS }) {
  ...
  const handleSaveItem = useCallback(async (payload) => {
    try {
      ...
      if (resp?.code === 201) {
        toast({ title: 'Solicitud creada', description: 'Se creó la solicitud de Triangulación.' });
      } else {
  ...
  const initialItem = useMemo(() => ({ tipoOperacion: { id: 'triangulacion', nombre: 'Triangulación' } }), []);

  return (
    <>
      <OperationModuleBase
        title="Triangulación"
        description="Listado y gestión de operaciones de triangulación."
        columns={operationsColumns}
        filterFields={FILTER_FIELDS}
        fetchService={fetchOperations}
        exportService={exportOperations}
        columnsExcel={columnsExcel}
        fileName="operaciones_triangulacion"
        defaultFilters={{ 'tipoOperacion.id': 'triangulacion' }}
      ...
      <OperationModal
        ...
        item={initialItem}
        title="Crear Solicitud Triangulación"
      />
    </>
  );
}
