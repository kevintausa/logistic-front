...
export default function FCLOperationPage() {
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <FCLOperationsSection />
    </div>
  );
}

function FCLOperationsSection() {
  ...
  const handleSaveItem = useCallback(async (payload) => {
    try {
      ...
      if (resp?.code === 201) {
        toast({ title: 'Solicitud creada', description: 'Se creó la solicitud FCL.' });
      } else {
  ...
  const initialItem = useMemo(() => ({ tipoOperacion: { id: 'importacion_fcl', nombre: 'Importación FCL' } }), []);

  return (
    <>
      <OperationModuleBase
        title="FCL"
        description="Listado y gestión de operaciones FCL."
        columns={operationsColumns}
        filterFields={FILTER_FIELDS}
        fetchService={fetchOperations}
        exportService={exportOperations}
        columnsExcel={columnsExcel}
        fileName="operaciones_fcl"
        defaultFilters={{ 'tipoOperacion.id': { $in: ['importacion_fcl', 'exportacion_fcl'] } }}
